...
export const startPaymentConsumer = async () => { // Hàm bất đồng bộ khởi động RabbitMQ Consumer để lắng nghe các message về thanh toán
    const channel = getChanel(); // Lấy channel RabbitMQ đã được kết nối

    channel.consume(process.env.PAYMENT_QUEUE!, async (msg) => { // Đăng ký lắng nghe hàng đợi (queue) thanh toán
        if (!msg) return; // Nếu message rỗng thì bỏ qua
