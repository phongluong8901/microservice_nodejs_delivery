...
export const startOrderReadyConsumer = async () => {                          // Khai báo hàm bất đồng bộ khởi động consumer lắng nghe queue
    ...
    channel.consume(process.env.ORDER_READY_QUEUE!, async (msg) => {         // Đăng ký nhận message từ queue, callback chạy mỗi khi có message mới đến
        if (!msg) return;                                                    // Nếu message trống thì bỏ qua
        try {
            ...
            const {
                ...
                restaurantId,
                location,
            } = event.data;                                                  // Lấy các thông tin orderId, restaurantId, và location từ dữ liệu sự kiện

            console.log("Searching for rider near: ", JSON.stringify(location)); // In log tọa độ đang tìm kiếm tài xế xung quanh

            const riders = await Rider.find({                                // Truy vấn MongoDB tìm các tài xế thỏa mãn điều kiện
            ...
            for (const rider of riders) {                                    // Duyệt qua danh sách các tài xế tìm được
                ...
                try {
                    await axios.post(`${process.env.REALTIME_SERVICE}/api/v1/internal/emit`, { // Gọi HTTP API tới microservice Realtime để gửi thông báo real-time
